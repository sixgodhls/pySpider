        ...
        void innerComplete(T value) {
            if (value != null) {
                while (true) {
                    ...
                    try {
                        value = ObjectHelper.requireNonNull(this.reducer.mo312apply(sp.first, sp.second), "The reducer returned a null value");
                    } catch (Throwable ex) {
    ...
    public static final class ParallelReduceFullInnerSubscriber<T> extends AtomicReference<Subscription> implements FlowableSubscriber<T> {
        ...
        public void onNext(T t) {
            if (!this.done) {
                ...
                try {
                    this.value = (T) ObjectHelper.requireNonNull(this.reducer.mo312apply(v, t), "The reducer returned a null value");
                } catch (Throwable ex) {
                    Exceptions.throwIfFatal(ex);
                    get().cancel();
                    onError(ex);
