...
final class Relay {
    ...
    private void writeHeader(ByteString prefix, long upstreamSize, long metadataSize) throws IOException {
        Buffer header = new Buffer();
        header.mo351write(prefix);
        header.mo359writeLong(upstreamSize);
        header.mo359writeLong(metadataSize);
        if (header.size() != 32) {
    ...
    private void writeMetadata(long upstreamSize) throws IOException {
        Buffer metadataBuffer = new Buffer();
        metadataBuffer.mo351write(this.metadata);
        FileOperator fileOperator = new FileOperator(this.file.getChannel());
    ...
    class RelaySource implements Source {
        ...
        /* JADX WARN: Code restructure failed: missing block: B:37:0x00e1, code lost:
            if (r21.this$0.buffer.size() <= r21.this$0.bufferMaxSize) goto L39;
         */
