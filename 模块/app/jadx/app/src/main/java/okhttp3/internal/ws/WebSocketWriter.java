...
final class WebSocketWriter {
    ...
    public void writeClose(int code, ByteString reason) throws IOException {
        ...
        if (code != 0 || reason != null) {
            ...
            buffer.mo361writeShort(code);
            if (reason != null) {
                buffer.mo351write(reason);
            }
    ...
    private void writeControlFrame(int opcode, ByteString payload) throws IOException {
        ...
        int b0 = opcode | 128;
        this.sinkBuffer.mo354writeByte(b0);
        if (!this.isClient) {
            this.sinkBuffer.mo354writeByte(length);
            this.sinkBuffer.mo351write(payload);
        } else {
            int b1 = length | 128;
            this.sinkBuffer.mo354writeByte(b1);
            this.random.nextBytes(this.maskKey);
            this.sinkBuffer.mo352write(this.maskKey);
            if (length > 0) {
    ...
    void writeMessageFrame(int formatOpcode, long byteCount, boolean isFirstFrame, boolean isFinal) throws IOException {
        ...
        if (byteCount <= 125) {
            this.sinkBuffer.mo354writeByte(b1 | ((int) byteCount));
        } else if (byteCount <= 65535) {
            this.sinkBuffer.mo354writeByte(b1 | 126);
            this.sinkBuffer.mo361writeShort((int) byteCount);
        } else {
            this.sinkBuffer.mo354writeByte(b1 | 127);
            this.sinkBuffer.mo359writeLong(byteCount);
        }
        if (this.isClient) {
            this.random.nextBytes(this.maskKey);
            this.sinkBuffer.mo352write(this.maskKey);
