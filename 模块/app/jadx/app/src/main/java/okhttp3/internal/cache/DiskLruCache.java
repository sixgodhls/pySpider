...
public final class DiskLruCache implements Closeable, Flushable {
    ...
    synchronized void rebuildJournal() throws IOException {
        ...
        writer.mo365writeUtf8(MAGIC).mo354writeByte(10);
        writer.mo365writeUtf8(VERSION_1).mo354writeByte(10);
        writer.mo355writeDecimalLong(this.appVersion).mo354writeByte(10);
        writer.mo355writeDecimalLong(this.valueCount).mo354writeByte(10);
        writer.mo354writeByte(10);
        for (Entry entry : this.lruEntries.values()) {
            if (entry.currentEditor != null) {
                writer.mo365writeUtf8(DIRTY).mo354writeByte(32);
                writer.mo365writeUtf8(entry.key);
                writer.mo354writeByte(10);
            } else {
                writer.mo365writeUtf8(CLEAN).mo354writeByte(32);
                writer.mo365writeUtf8(entry.key);
    ...
    public synchronized Snapshot get(String key) throws IOException {
        ...
        if (entry != null && entry.readable) {
            ...
            this.journalWriter.mo365writeUtf8(READ).mo354writeByte(32).mo365writeUtf8(key).mo354writeByte(10);
    ...
    synchronized Editor edit(String key, long expectedSequenceNumber) throws IOException {
        ...
        if (expectedSequenceNumber == -1 || (entry != null && entry.sequenceNumber == expectedSequenceNumber)) {
            ...
            if (!this.mostRecentTrimFailed && !this.mostRecentRebuildFailed) {
                this.journalWriter.mo365writeUtf8(DIRTY).mo354writeByte(32).mo365writeUtf8(key).mo354writeByte(10);
    ...
    synchronized void completeEdit(Editor editor, boolean success) throws IOException {
        ...
        if (entry.currentEditor == editor) {
            ...
            if (entry.readable || success) {
                entry.readable = true;
                this.journalWriter.mo365writeUtf8(CLEAN).mo354writeByte(32);
                this.journalWriter.mo365writeUtf8(entry.key);
                entry.writeLengths(this.journalWriter);
                this.journalWriter.mo354writeByte(10);
                if (success) {
                ...
                this.journalWriter.mo365writeUtf8(REMOVE).mo354writeByte(32);
                this.journalWriter.mo365writeUtf8(entry.key);
                this.journalWriter.mo354writeByte(10);
            }
            this.journalWriter.flush();
    ...
    boolean removeEntry(Entry entry) throws IOException {
        ...
        this.journalWriter.mo365writeUtf8(REMOVE).mo354writeByte(32).mo365writeUtf8(entry.key).mo354writeByte(10);
        this.lruEntries.remove(entry.key);
        if (journalRebuildRequired()) {
            this.executor.execute(this.cleanupRunnable);
        }
    ...
    public final class Entry {
        ...
        void writeLengths(BufferedSink writer) throws IOException {
            ...
            for (long length : this.lengths) {
                writer.mo354writeByte(32).mo355writeDecimalLong(length);
            }
        }
