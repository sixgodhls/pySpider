...
public final class MultipartBody extends RequestBody {
    ...
    private long writeOrCountBytes(@Nullable BufferedSink sink, boolean countBytes) throws IOException {
        ...
        for (int p = 0; p < partCount; p++) {
            ...
            sink2.mo414write(DASHDASH);
            sink2.mo413write(this.boundary);
            sink2.mo414write(CRLF);
            if (headers != null) {
                int headerCount = headers.size();
                for (int h = 0; h < headerCount; h++) {
                    sink2.mo427writeUtf8(headers.name(h)).mo414write(COLONSPACE).mo427writeUtf8(headers.value(h)).mo414write(CRLF);
                }
            }
            MediaType contentType = body.contentType();
            if (contentType != null) {
                sink2.mo427writeUtf8("Content-Type: ").mo427writeUtf8(contentType.toString()).mo414write(CRLF);
            }
            long contentLength = body.contentLength();
            if (contentLength != -1) {
                sink2.mo427writeUtf8("Content-Length: ").mo417writeDecimalLong(contentLength).mo414write(CRLF);
            } else if (countBytes) {
