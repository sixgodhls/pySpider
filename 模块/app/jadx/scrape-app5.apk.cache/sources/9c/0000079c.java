...
public final class Hpack {
    ...
    static final class Reader {
        ...
        ByteString readByteString() throws IOException {
            ...
            if (huffmanDecode) {
                return ByteString.m2of(Huffman.get().decode(this.source.readByteArray(length)));
            }
    ...
    static final class Writer {
        ...
        void writeInt(int value, int prefixMask, int bits) {
            if (value < prefixMask) {
                this.out.mo416writeByte(bits | value);
                return;
            }
            this.out.mo416writeByte(bits | prefixMask);
            int value2 = value - prefixMask;
            while (value2 >= 128) {
                int b = value2 & Hpack.PREFIX_7_BITS;
                this.out.mo416writeByte(b | 128);
                value2 >>>= 7;
            }
            this.out.mo416writeByte(value2);
        }
        ...
        void writeByteString(ByteString data) throws IOException {
            if (this.useCompression && Huffman.get().encodedLength(data) < data.size()) {
                ...
                this.out.mo413write(huffmanBytes);
                return;
            }
            writeInt(data.size(), Hpack.PREFIX_7_BITS, 0);
            this.out.mo413write(data);
        }
