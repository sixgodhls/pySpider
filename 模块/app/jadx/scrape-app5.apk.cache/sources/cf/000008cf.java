...
public final class Cache implements Closeable, Flushable {
    ...
    public static final class Entry {
        ...
        public void writeTo(DiskLruCache.Editor editor) throws IOException {
            ...
            sink.mo427writeUtf8(this.url).mo416writeByte(10);
            sink.mo427writeUtf8(this.requestMethod).mo416writeByte(10);
            sink.mo417writeDecimalLong(this.varyHeaders.size()).mo416writeByte(10);
            int size = this.varyHeaders.size();
            for (int i = 0; i < size; i++) {
                sink.mo427writeUtf8(this.varyHeaders.name(i)).mo427writeUtf8(": ").mo427writeUtf8(this.varyHeaders.value(i)).mo416writeByte(10);
            }
            sink.mo427writeUtf8(new StatusLine(this.protocol, this.code, this.message).toString()).mo416writeByte(10);
            sink.mo417writeDecimalLong(this.responseHeaders.size() + 2).mo416writeByte(10);
            int size2 = this.responseHeaders.size();
            for (int i2 = 0; i2 < size2; i2++) {
                sink.mo427writeUtf8(this.responseHeaders.name(i2)).mo427writeUtf8(": ").mo427writeUtf8(this.responseHeaders.value(i2)).mo416writeByte(10);
            }
            sink.mo427writeUtf8(SENT_MILLIS).mo427writeUtf8(": ").mo417writeDecimalLong(this.sentRequestMillis).mo416writeByte(10);
            sink.mo427writeUtf8(RECEIVED_MILLIS).mo427writeUtf8(": ").mo417writeDecimalLong(this.receivedResponseMillis).mo416writeByte(10);
            if (isHttps()) {
                sink.mo416writeByte(10);
                sink.mo427writeUtf8(this.handshake.cipherSuite().javaName()).mo416writeByte(10);
                writeCertList(sink, this.handshake.peerCertificates());
                writeCertList(sink, this.handshake.localCertificates());
                sink.mo427writeUtf8(this.handshake.tlsVersion().javaName()).mo416writeByte(10);
            }
            sink.close();
        }
        ...
        private List<Certificate> readCertificateList(BufferedSource source) throws IOException {
            ...
            try {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    bytes.mo413write(ByteString.decodeBase64(line));
        ...
        private void writeCertList(BufferedSink sink, List<Certificate> certificates) throws IOException {
            try {
                sink.mo417writeDecimalLong(certificates.size()).mo416writeByte(10);
                int size = certificates.size();
                for (int i = 0; i < size; i++) {
                    byte[] bytes = certificates.get(i).getEncoded();
                    String line = ByteString.m2of(bytes).base64();
                    sink.mo427writeUtf8(line).mo416writeByte(10);
                }
